...
/// This struct holds the current lighting mode, and the data for global animations.
...
pub struct PCPanel {
    ...
    pub individual_led_data: Vec<ColorMode>,
}

/* -------- Implementations -------- */

impl ColorMode {
    /// Helper function to map LED mode to binary value
...
impl LedMode {
    /// Helper function to map command type to binary value
    pub fn to_byte(&self) -> u8 {
        match self {
...
impl AnimationType {
    /// Helper function to map animation to binary value
    pub fn to_byte(&self) -> u8 {
        match self {
...
impl PCPanel {
    ...
    pub fn new() -> PCPanel {
        ...
        // Fill in LED Data
        for _i in 0..4 {
            panel.individual_led_data.push(ColorMode::StaticColor {
                r: 255,
                g: 255,
                b: 255,
            });
        ...
        match self.global_led_data.led_mode {
            ...
            LedMode::CustomKnob => {
                for i in 0..self.knob_values.len() {
                    ...
                    message.push(self.individual_led_data[i].to_byte());
                    // Push custom color data
                    match &self.individual_led_data[i] {
                        ColorMode::StaticColor { r, g, b } => {
                            message.extend_from_slice(&[*r, *g, *b, 0, 0, 0]);
                        }
                        ColorMode::GradientColor {
                            start_r,
                            start_g,
